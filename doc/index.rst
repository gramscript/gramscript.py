...
gramscript helps you build applications for `Telegram Bot API <https://core.telegram.org/bots>`_.
...
pip::
    ...
    $ pip install gramscript
    $ pip install gramscript --upgrade  # UPGRADE

easy_install::

    $ easy_install gramscript
    $ easy_install --upgrade gramscript  # UPGRADE
...
With the token in hand, you can start using gramscript to access the bot account.
...
::
    ...
    >>> import gramscript
    >>> bot = gramscript.Bot('***** PUT YOUR TOKEN HERE *****')
    >>> bot.getMe()
...
In gramscript, **Bot API objects are represented as dictionary.**
...
It is troublesome to keep checking messages while managing ``offset``. Let gramscript
...
almost always have to extract them. Use :func:`gramscript.glance` to extract
...
The module :mod:`gramscript.namedtuple` provides namedtuple classes for easier
...
Regardless of the type of objects received, gramscript generically calls them
...
Use :func:`gramscript.flavor` to check a message's flavor.
...
- How I use namedtuple to construct
  ...
  object
- :func:`gramscript.glance` works on any type of messages. Just give it the flavor.
...
Python 3.5+ has its own ``asyncio`` module. gramscript supports that, too.
...
gramscript's async version basically mirrors the traditional version. Main differences are:
...
Async version is under module :mod:`gramscript.aio`. I duplicate the message counter example
