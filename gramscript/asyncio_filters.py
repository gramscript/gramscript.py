...
from gramscript.asyncio_handler_backends import State
...
class TextFilter:
    ...
    def __init__(self,
        ...
        to_check = sum((pattern is not None for pattern in (
        ...
        self.starts_with = self._check_iterable(
            starts_with, filter_name='starts_with')
        self.ends_with = self._check_iterable(
            ends_with, filter_name='ends_with')
    ...
    async def check(self, obj: Union[types.Message, types.CallbackQuery, types.InlineQuery, types.Poll]):
        ...
        if self.ignore_case:
            text = text.lower()
            def prepare_func(string): return str(string).lower()
        else:
            prepare_func = str
        ...
        if self.starts_with:
            result = any([text.startswith(prepare_func(i))
                         for i in self.starts_with])
            if result:
                return True
