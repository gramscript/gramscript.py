...
import gramscript.util
import gramscript.types


# storages
from gramscript.asyncio_storage import StateMemoryStorage, StatePickleStorage, StateStorageBase
from gramscript.asyncio_handler_backends import BaseMiddleware, CancelUpdate, SkipHandler, State

from inspect import signature

from gramscript import logger

from gramscript import util, types, asyncio_helper
import asyncio
from gramscript import asyncio_filters
...
class AsyncTeleBot:
    ...
    https://github.com/gramscript/gramscript.py/tree/master/examples
    ...
    async def _process_updates(self, handlers, messages, update_type):
        ...
        for message in messages:
            ...
            tasks.append(self._run_middlewares_and_handlers(
                handlers, message, middleware, update_type))
                ...
                for handler in handlers:
                    ...
                    if len(params) == 1:
                        ...
                        if len(data_copy) > len(params) - 1:  # remove the message parameter
                            logger.error("You are passing more data than the handler needs. Check your handler: {}".format(
                                handler['function']))
                            return
            ...
            for middleware in middlewares:
                if middleware.update_sensitive:
                    if hasattr(middleware, f'post_process_{update_type}'):
                        ...
                        logger.error('Middleware {} does not have post_process_{} method. post_process function execution was skipped.'.format(
                            middleware.__class__.__name__, update_type))
                else:
                    await middleware.post_process(message, data, handler_error)
    # update handling
    ...
    async def process_middlewares(self, update_type):
        ...
        if self.middlewares:
            middlewares = [
                middleware for middleware in self.middlewares if update_type in middleware.update_types]
            return middlewares
        return None
