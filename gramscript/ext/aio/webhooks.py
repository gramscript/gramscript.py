...
from gramscript.types import Update
import asyncio
...
class AsyncWebhookListener:
    ...
    def _prepare_endpoint_urls(self):
        self.app.add_api_route(endpoint=self.process_update,
                               path=self._url_path, methods=["POST"])
    ...
    async def process_update(self, request: Request, update: dict):
        ...
        if request.headers.get('content-type') == 'application/json':
            json_string = update
            asyncio.create_task(self._bot.process_new_updates(
