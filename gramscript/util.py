...
from gramscript.api import ApiTelegramException
...
def smart_split(text: str, chars_per_string: int = MAX_MESSAGE_LENGTH) -> List[str]:
    ...
    chars_per_string = min(chars_per_string, MAX_MESSAGE_LENGTH)
    parts = []
    while True:
...
def user_link(user: User, include_id: bool = False) -> str:
    """
...
def webhook_google_functions(bot, request):
    ...
    if request.is_json:
        try:
            ...
            update = Update.de_json(request_json)
...
def antiflood(function, *args, **kwargs):
    ...
    from gramscript.util import antiflood
