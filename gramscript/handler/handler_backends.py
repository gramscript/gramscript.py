    ...
    def dump_handlers(handlers, filename, file_mode="wb"):
        ...
        with open(f"{filename}.tmp", file_mode) as file:
            if CUSTOM_SERIALIZER is None:
                ...
                CUSTOM_SERIALIZER.dump(handlers, file)
        ...
        if os.path.isfile(filename) and os.path.getsize(filename) > 0:
            with open(filename, "rb") as file:
                if (CUSTOM_SERIALIZER is None):
                    ...
                    handlers = CUSTOM_SERIALIZER.load(file)
...
class RedisHandlerBackend(HandlerBackend):
    def __init__(self, handlers=None, host='localhost', port=6379, db=0, prefix='gramscript', password=None):
...
class StateMemory:
    ...
    def current_state(self, chat_id):
        """Current state"""
        return self._states[chat_id]['state'] if chat_id in self._states else False
...
class StateFile:
    ...
    def current_state(self, chat_id):
        """Current state."""
        states_data = self._read_data()
        return states_data[chat_id]['state'] if chat_id in states_data else False
