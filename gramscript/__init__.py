...
from gramscript.api import ApiException
from gramscript.asyncio.helper import _process_request
from gramscript.filters import AdvancedCustomFilter, SimpleCustomFilter, StateFilter
from gramscript.get import get_me, get_file, log_out, close, download_file, get_file_url
from gramscript.get.get import get_updates
from gramscript.handler import FileHandlerBackend, MemoryHandlerBackend
from gramscript.inline import answer_callback_query, answer_inline_query
from gramscript.types.chat_and_user import Chat, ChatInviteLink, ChatMember, ChatPermissions, Update, User, get_user_profile_photos, delete_chat_sticker_set, get_chat, get_chat_administrators, get_chat_member, get_chat_member_count, leave_chat, set_chat_sticker_set
from gramscript.types.command import BotCommand, BotCommandScope
from gramscript.types.game import GameHighScore
from gramscript.types.keyboard import ReplyKeyboardMarkup, ReplyKeyboardRemove, InlineKeyboardMarkup
from gramscript.types.chat_and_user import copy_message, delete_message, edit_message_caption, edit_message_live_location, edit_message_media, edit_message_reply_markup, edit_message_text, forward_message, get_game_high_scores, send_animation, send_audio, send_chat_action, send_contact, send_data, send_dice, send_game, send_location, send_media_group, send_message, send_photo, send_poll, send_venue, send_video, send_video_note, send_voice, set_game_score, stop_message_live_location, stop_poll, approve_chat_join_request, ban_chat_member, ban_chat_sender_chat, create_chat_invite_link, decline_chat_join_request, delete_chat_photo, delete_my_commands, edit_chat_invite_link, export_chat_invite_link, pin_chat_message, promote_chat_member, restrict_chat_member, revoke_chat_invite_link, set_chat_administrator_custom_title, set_chat_description, set_chat_permissions, set_chat_photo, set_chat_title, set_my_commands, unban_chat_member, unban_chat_sender_chat, unpin_all_chat_messages, unpin_chat_message, add_sticker_to_set, create_new_sticker_set, delete_sticker_from_set, get_sticker_set, set_sticker_position_in_set, set_sticker_set_thumb, upload_sticker_file
from gramscript.types.media import File, InputMediaAudio, InputMediaDocument, InputMediaPhoto, InputMediaVideo, UserProfilePhotos
from gramscript.types.message import ForceReply, Message, MessageEntity, MessageID
from gramscript.types.payment import LabeledPrice, ShippingOption, answer_pre_checkout_query, answer_shipping_query
from gramscript.types.poll import Poll
from gramscript.types.sticker import MaskPosition, StickerSet
from gramscript.webhook import WebhookInfo, delete_webhook, get_webhook_info, set_webhook
from .util import *
try:
...
class gramscript:
    ...
    async def set_webhook(self, url=None, certificate=None, max_connections=None, allowed_updates=None, ip_address=None, drop_pending_updates=None, timeout=None):
        ...
        return await _process_request(self, method_url, params=payload, files=files)
    ...
    def user(self) -> User:
        ...
        if not hasattr(self, "_user"):
            self._user = User.de_json(self.get_me())
        return self._user
    ...
    def enable_saving_states(self, filename="./.state-save/states.pkl"):
        ...
        self.current_states = StateFilter(filename=filename)
    ...
    def set_webhook(self, url=None, certificate=None, max_connections=None, allowed_updates=None, ip_address=None,
        ...
        :param allowed_updates: A JSON-serialized list of the update types you want your bot to receive.
            ...
            of these  See Update for a complete list of available update 
        ...
        :return:
        """
        return set_webhook(self.token, url, certificate, max_connections, allowed_updates, ip_address,
    ...
    def delete_webhook(self, drop_pending_updates=None, timeout=None):
        ...
        :return: bool
        """
        return delete_webhook(self.token, drop_pending_updates, timeout)
    ...
    def get_webhook_info(self, timeout: Optional[int] = None):
        ...
        """
        result = get_webhook_info(self.token, timeout)
    ...
    def get_updates(self, offset: Optional[int] = None, limit: Optional[int] = None,
        ...
        """
        json_updates = get_updates(
            self.token, offset, limit, timeout, allowed_updates, long_polling_timeout)
        return [Update.de_json(ju) for ju in json_updates]
    ...
    def process_new_my_chat_member(self, my_chat_members):
        self._notify_command_handlers(
            self.my_chat_member_handlers, my_chat_members)
    ...
    def process_new_chat_join_request(self, chat_join_request):
        self._notify_command_handlers(
            self.chat_join_request_handlers, chat_join_request)
    ...
    def infinity_polling(self, timeout: int = 20, skip_pending: bool = False, long_polling_timeout: int = 20, logger_level=logging.ERROR,
        ...
        :param allowed_updates: A list of the update types you want your bot to receive.
            ...
            See update_types for a complete list of available update  
    ...
    def polling(self, non_stop: bool = False, skip_pending=False, interval: int = 0, timeout: int = 20,
        ...
        if self.threaded:
            self.__threaded_polling(
                non_stop, interval, timeout, long_polling_timeout, allowed_updates)
        else:
    ...
    def __threaded_polling(self, non_stop=False, interval=0, timeout=None, long_polling_timeout=None, allowed_updates=None):
        self.__extracted_from___non_threaded_polling_2()
        ...
        while not self.__stop_polling.wait(interval):
            ...
            try:
                ...
                if self.exception_handler is not None:
                    ...
                    handled = False
                if not handled:
                    self.__extracted_from___threaded_polling_58(polling_thread)
                    raise e
                else:
                    polling_thread.clear_exceptions()
                    self.worker_pool.clear_exceptions()
    ...
    def __extracted_from___threaded_polling_58(self, polling_thread):
        polling_thread.stop()
        polling_thread.clear_exceptions()
        self.worker_pool.clear_exceptions()

    # TODO Rename this here and in `__threaded_polling` and `__non_threaded_polling`
    ...
    # TODO Rename this here and in `__threaded_polling` and `__non_threaded_polling`
    def __extracted_from___non_threaded_polling_2(self):
    ...
    def get_me(self) -> User:
        ...
        """
        result = get_me(self.token)
        return User.de_json(result)
    ...
    def get_file(self, file_id: str) -> File:
        ...
        """
        return File.de_json(get_file(self.token, file_id))

    def get_file_url(self, file_id: str) -> str:
        return get_file_url(self.token, file_id)

    def download_file(self, file_path: str) -> bytes:
        return download_file(self.token, file_path)
    ...
    def get_chat(self, chat_id: Union[int, str]) -> Chat:
        ...
        :return:
        """
        result = get_chat(self.token, chat_id)
    ...
    def leave_chat(self, chat_id: Union[int, str]) -> bool:
        ...
        :return:
        """
        return leave_chat(self.token, chat_id)
    ...
    def get_chat_administrators(self, chat_id: Union[int, str]) -> List[ChatMember]:
        ...
        :return:
        """
        result = get_chat_administrators(self.token, chat_id)
        return [ChatMember.de_json(r) for r in result]
    ...
    def get_chat_members_count(self, chat_id: Union[int, str]) -> int:
        ...
        """
        logger.info(
            'get_chat_members_count is deprecated. Use get_chat_member_count instead.')

        return get_chat_member_count(self.token, chat_id)
    ...
    def set_chat_sticker_set(self, chat_id: Union[int, str], sticker_set_name: str) -> StickerSet:
        """
        ...
        :return:
        """
        return set_chat_sticker_set(self.token, chat_id, sticker_set_name)
    ...
    def delete_chat_sticker_set(self, chat_id: Union[int, str]) -> bool:
        ...
        :return:
        """
        return delete_chat_sticker_set(self.token, chat_id)
    ...
    def get_chat_member(self, chat_id: Union[int, str], user_id: int) -> ChatMember:
        ...
        :return:
        """
        result = get_chat_member(self.token, chat_id, user_id)
    ...
    def send_message(
        ...
        use the `split_string` or `smart_split` function in py.
    ...
    def forward_message(
        ...
        """
        return Message.de_json(
            forward_message(self.token, chat_id, from_chat_id, message_id, disable_notification, timeout, protect_content))
    ...
    def copy_message(
        ...
        return MessageID.de_json(
            copy_message(self.token, chat_id, from_chat_id, message_id, caption, parse_mode, caption_entities,
    ...
    def delete_message(self, chat_id: Union[int, str], message_id: int,
        ...
        :return: API reply.
        """
        return delete_message(self.token, chat_id, message_id, timeout)
    ...
    def send_sticker(
            ...
            allow_sending_without_reply: Optional[bool] = None,
            protect_content: Optional[bool] = None,
            data: Union[Any, str] = None) -> Message:
        """
    ...
    def send_video(
            ...
            reply_markup: Optional[REPLY_MARKUP_TYPES] = None,
            timeout: Optional[int] = None,
            data: Optional[Union[Any, str]] = None) -> Message:
        """
    ...
    def send_media_group(
        ...
        result = send_media_group(
            ...
            allow_sending_without_reply, protect_content)
        return [Message.de_json(msg) for msg in result]
    ...
    def edit_message_live_location(
            ...
            horizontal_accuracy: Optional[float] = None,
            heading: Optional[int] = None,
            proximity_alert_radius: Optional[int] = None) -> Message:
        """
    ...
    def send_chat_action(
        ...
        """
        return send_chat_action(self.token, chat_id, action, timeout)
    ...
    def kick_chat_member(
        ...
        """
        logger.info(
            'kick_chat_member is deprecated. Use ban_chat_member instead.')
        return ban_chat_member(self.token, chat_id, user_id, until_date, revoke_messages)
    ...
    def unban_chat_member(
        ...
        """
        return unban_chat_member(self.token, chat_id, user_id, only_if_banned)
    ...
    def set_chat_administrator_custom_title(
        ...
        :return: True on success.
        """
        return set_chat_administrator_custom_title(self.token, chat_id, user_id, custom_title)
    ...
    def ban_chat_sender_chat(self, chat_id: Union[int, str], sender_chat_id: Union[int, str]) -> bool:
        ...
        :return: True on success.
        """
        return ban_chat_sender_chat(self.token, chat_id, sender_chat_id)
    ...
    def unban_chat_sender_chat(self, chat_id: Union[int, str], sender_chat_id: Union[int, str]) -> bool:
        ...
        :return: True on success.
        """
        return unban_chat_sender_chat(self.token, chat_id, sender_chat_id)

    def set_chat_permissions(
            self, chat_id: Union[int, str], permissions: ChatPermissions) -> bool:
        """
        ...
        :return: True on success
        """
        return set_chat_permissions(self.token, chat_id, permissions)
    ...
    def create_chat_invite_link(
            ...
            creates_join_request: Optional[bool] = None) -> ChatInviteLink:
        """
    ...
    def edit_chat_invite_link(
        ...
        return ChatInviteLink.de_json(
            edit_chat_invite_link(
                self.token, chat_id, name, invite_link, expire_date, member_limit, creates_join_request)
        )
    ...
    def revoke_chat_invite_link(
        ...
        """
        return ChatInviteLink.de_json(
            revoke_chat_invite_link(self.token, chat_id, invite_link)
        )
    ...
    def export_chat_invite_link(self, chat_id: Union[int, str]) -> str:
        ...
        """
        return export_chat_invite_link(self.token, chat_id)
    ...
    def approve_chat_join_request(self, chat_id: Union[str, int], user_id: Union[int, str]) -> bool:
        ...
        :return: True on success.
        """
        return approve_chat_join_request(self.token, chat_id, user_id)
    ...
    def decline_chat_join_request(self, chat_id: Union[str, int], user_id: Union[int, str]) -> bool:
        ...
        :return: True on success.
        """
        return decline_chat_join_request(self.token, chat_id, user_id)
    ...
    def set_chat_photo(self, chat_id: Union[int, str], photo: Any) -> bool:
        ...
        :return:
        """
        return set_chat_photo(self.token, chat_id, photo)
    ...
    def delete_chat_photo(self, chat_id: Union[int, str]) -> bool:
        ...
        :param chat_id: Int or Str: Unique identifier for the target chat or username of the target channel
            (in the format @channelusername)
        """
        return delete_chat_photo(self.token, chat_id)

    def get_my_commands(self, scope: Optional[BotCommandScope],
                        language_code: Optional[str]) -> List[BotCommand]:
        """
        ...
        """
        result = set_my_commands(self.token, scope, language_code)
        return [BotCommand.de_json(cmd) for cmd in result]

    def set_my_commands(self, commands: List[BotCommand],
        ...
        :return:
        """
        return set_my_commands(self.token, commands, scope, language_code)

    def delete_my_commands(self, scope: Optional[BotCommandScope] = None,
        ...
        :param language_code: A two-letter ISO 639-1 language code. If empty, 
            ...
            for whose language there are no dedicated commands
        """
        return delete_my_commands(self.token, scope, language_code)
    ...
    def set_chat_title(self, chat_id: Union[int, str], title: str) -> bool:
        ...
        :return:
        """
        return set_chat_title(self.token, chat_id, title)
    ...
    def set_chat_description(self, chat_id: Union[int, str], description: Optional[str] = None) -> bool:
        ...
        :return: True on success.
        """
        return set_chat_description(self.token, chat_id, description)
    ...
    def pin_chat_message(
        ...
        :return:
        """
        return pin_chat_message(self.token, chat_id, message_id, disable_notification)
    ...
    def unpin_chat_message(self, chat_id: Union[int, str], message_id: Optional[int] = None) -> bool:
        ...
        :return:
        """
        return unpin_chat_message(self.token, chat_id, message_id)
    ...
    def unpin_all_chat_messages(self, chat_id: Union[int, str]) -> bool:
        ...
        :return:
        """
        return unpin_all_chat_messages(self.token, chat_id)
    ...
    def edit_message_text(self, text: str, chat_id: Optional[Union[int, str]] = None, message_id: Optional[int] = None, inline_message_id: Optional[str] = None, parse_mode: Optional[str] = None, entities: Optional[List[MessageEntity]] = None, disable_web_page_preview: Optional[bool] = None, reply_markup: Optional[REPLY_MARKUP_TYPES] = None) -> Union[Message, bool]:
        ...
        result = edit_message_text(
            self.token, text, chat_id, message_id, inline_message_id, parse_mode, entities, disable_web_page_preview, reply_markup)

        return result if type(result) == bool else Message.de_json(result)

    def edit_message_media(self, media: Any, chat_id: Optional[Union[int, str]] = None, message_id: Optional[int] = None, inline_message_id: Optional[str] = None, reply_markup: Optional[REPLY_MARKUP_TYPES] = None) -> Union[Message, bool]:
        """
    ...
    def edit_message_reply_markup(self, chat_id: Optional[Union[int, str]] = None, message_id: Optional[int] = None, inline_message_id: Optional[str] = None, reply_markup: Optional[REPLY_MARKUP_TYPES] = None) -> Union[Message, bool]:
        """
    ...
    def set_game_score(self, user_id: Union[int, str], score: int, force: Optional[bool] = None, chat_id: Optional[Union[int, str]] = None, message_id: Optional[int] = None, inline_message_id: Optional[str] = None, disable_edit_message: Optional[bool] = None) -> Union[Message, bool]:
        """
        ...
        :return:
        """
        result = set_game_score(
            self.token, user_id, score, force, disable_edit_message, chat_id, message_id, inline_message_id)
    ...
    def get_game_high_scores(
            ...
            message_id: Optional[int] = None,
            inline_message_id: Optional[str] = None) -> List[GameHighScore]:
        """
    ...
    def stop_poll(
            self, chat_id: Union[int, str], message_id: int,
            reply_markup: Optional[REPLY_MARKUP_TYPES] = None) -> Poll:
        """
        Stops poll
        :param chat_id:
        :param message_id:
        ...
        :return:
        """
        return Poll.de_json(stop_poll(self.token, chat_id, message_id, reply_markup))
    ...
    def answer_shipping_query(
        ...
        :return:
        """
        return answer_shipping_query(self.token, shipping_query_id, ok, shipping_options, error_message)
    ...
    def answer_pre_checkout_query(
        ...
        :return:
        """
        return answer_pre_checkout_query(self.token, pre_checkout_query_id, ok, error_message)

    def edit_message_caption(self, caption: str, chat_id: Optional[Union[int, str]] = None, message_id: Optional[int] = None, inline_message_id: Optional[str] = None, parse_mode: Optional[str] = None, caption_entities: Optional[List[MessageEntity]] = None, reply_markup: Optional[REPLY_MARKUP_TYPES] = None) -> Union[Message, bool]:
        """
    ...
    def answer_inline_query(
        ...
        """
        return answer_inline_query(self.token, inline_query_id, results, cache_time, is_personal, next_offset,
    ...
    def answer_callback_query(
        ...
        :return:
        """
        return answer_callback_query(self.token, callback_query_id, text, show_alert, url, cache_time)
    ...
    def set_sticker_set_thumb(
        ...
        """
        return set_sticker_set_thumb(self.token, name, user_id, thumb)
    ...
    def get_sticker_set(self, name: str) -> StickerSet:
        ...
        :return:
        """
        result = get_sticker_set(self.token, name)
        return StickerSet.de_json(result)

    def upload_sticker_file(self, user_id: int, png_sticker: Union[Any, str]) -> File:
        """
    ...
    def create_new_sticker_set(
            ...
            mask_position: Optional[MaskPosition] = None) -> bool:
        """
    ...
    def set_sticker_position_in_set(self, sticker: str, position: int) -> bool:
        ...
        :return:
        """
        return set_sticker_position_in_set(self.token, sticker, position)
    ...
    def delete_sticker_from_set(self, sticker: str) -> bool:
        ...
        :return:
        """
        return delete_sticker_from_set(self.token, sticker)

    def register_for_reply(
            self, message: Message, callback: Callable, *args, **kwargs) -> None:
        """
    ...
    def _notify_reply_handlers(self, new_messages) -> None:
        ...
        for message in new_messages:
            if hasattr(message, "reply_to_message") and message.reply_to_message is not None:
                if handlers := self.reply_backend.get_handlers(message.reply_to_message.message_id):
    ...
    def _notify_next_handlers(self, new_messages):
        ...
        for i, message in enumerate(new_messages):
            need_pop = False
            if handlers := self.next_step_backend.get_handlers(message.chat.id):
                for handler in handlers:
                    need_pop = True
                    self._exec_task(
                        handler["callback"], message, *handler["args"], **handler["kwargs"])
    ...
    def add_middleware_handler(self, handler, update_types=None):
        ...
        if not ENABLE_MIDDLEWARE:
            raise RuntimeError(
                "Middleware is not enabled. Use ENABLE_MIDDLEWARE.")
    ...
    def message_handler(self, commands=None, regexp=None, func=None, content_types=None, chat_types=None, **kwargs):
        ...
        :param content_types: Supported message content  Must be a list. Defaults to ['text'].
    ...
    def _test_filter(self, message_filter, filter_value, message):
        ...
        if message_filter == 'content_types':
            ...
            return message.content_type == 'text' and extract_command(message.text) in filter_value
