# -*- coding: utf-8 -*-
from gramscript.custom_filters import SimpleCustomFilter, AdvancedCustomFilter
from gramscript.handler_backends import HandlerBackend, MemoryHandlerBackend, FileHandlerBackend, BaseMiddleware, CancelUpdate, SkipHandler, State
from gramscript import apihelper, util, types
import inspect
...
# storage
from gramscript.storage import StatePickleStorage, StateMemoryStorage, StateStorageBase
...
class TeleBot:
    ...
    def run_webhooks(self,
        ...
        # webhooks module
        try:
            from gramscript.ext.sync import SyncWebhookListener
            ...
            if not self.use_class_middlewares:
                for message_handler in handlers:
                    if self._test_message_handler(message_handler, message):
                        self._exec_task(
                            message_handler['function'], message, pass_bot=message_handler['pass_bot'], task_type='handler')
                        break
            else:
                middleware = self._check_middleware(update_type)
                self._exec_task(self._run_middlewares_and_handler, message,
                                handlers=handlers, middlewares=middleware, update_type=update_type)
                return
