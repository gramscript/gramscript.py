...
import gramscript
from gramscript.types.media.media import InputMedia
from gramscript.types.object import JsonSerializable
from gramscript.types.poll.poll import PollOption
    ...
    async with await session_manager._get_new_session() as session:
        while not got_result and current_try < MAX_RETRIES-1:
            ...
            try:
                ...
                logger.debug("Request: method={0} url={1} params={2} files={3} request_timeout={4} current_try={5}".format(
                    method, url, params, files, request_timeout, current_try).replace(token, token.split(':')[0] + ":{TOKEN}"))
                ...
                pass
        if not got_result:
            raise RequestTimeout("Request timeout. Request: method={0} url={1} params={2} files={3} request_timeout={4}".format(
...
async def _convert_markup(markup):
    return markup.to_json() if isinstance(markup, JsonSerializable) else markup
...
async def send_media_group(token, chat_id, media, disable_notification=None, reply_to_message_id=None, timeout=None, allow_sending_without_reply=None, protect_content=None):
    ...
    if protect_content is not None:
        payload['protect_content'] = protect_content
    return await _process_request(token, method_url, params=payload, method='post' if files else 'get', files=files or None)
...
async def send_video(token, chat_id, data, duration=None, caption=None, reply_to_message_id=None, reply_markup=None, parse_mode=None, supports_streaming=None, disable_notification=None, timeout=None, thumb=None, width=None, height=None, caption_entities=None, allow_sending_without_reply=None, protect_content=None):
...
async def set_webhook(token, url=None, certificate=None, max_connections=None, allowed_updates=None, ip_address=None, drop_pending_updates=None, timeout=None):
...
async def send_video_note(token, chat_id, data, duration=None, length=None, reply_to_message_id=None, reply_markup=None, disable_notification=None, timeout=None, thumb=None, allow_sending_without_reply=None, protect_content=None):
...
async def send_audio(token, chat_id, audio, caption=None, duration=None, performer=None, title=None, reply_to_message_id=None, reply_markup=None, parse_mode=None, disable_notification=None, timeout=None, thumb=None, caption_entities=None, allow_sending_without_reply=None, protect_content=None):
...
async def _convert_poll_options(poll_options):
    if poll_options is None:
        ...
        return []
    elif isinstance(poll_options[0], str) or not isinstance(poll_options[0], PollOption):
