...
from gramscript import TeleBot, types
from gramscript.custom_filters import TextFilter, TextMatchFilter, IsReplyFilter
...
# STark, sTeve, stONE
@bot.message_handler(text=TextFilter(starts_with='st', ignore_case=True))
def starts_with_handler_ignore_case(message: types.Message):
    bot.send_message(message.chat.id, message.text + ' ignore case')


# wednesday, SUNday, WeekDay
@bot.message_handler(text=TextFilter(ends_with='ay'))
def ends_with_handler(message: types.Message):
    bot.send_message(message.chat.id, message.text)


# wednesdAY, sundAy, WeekdaY
@bot.message_handler(text=TextFilter(ends_with='ay', ignore_case=True))
    ...
    keyboard = types.InlineKeyboardMarkup(
        keyboard=[
            [types.InlineKeyboardButton(
                text='callback data', callback_data='example')],
            [types.InlineKeyboardButton(
...
def send_poll(message: types.Message):
    bot.send_poll(message.chat.id, question='When do you prefer to work?', options=[
                  'Morning', 'Night'])
    bot.send_poll(message.chat.id, question='WHEN DO you pRefeR to worK?', options=[
                  'Morning', 'Night'])
