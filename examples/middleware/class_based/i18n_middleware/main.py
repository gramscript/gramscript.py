...
from gramscript import types, StateMemoryStorage
from gramscript.custom_filters import TextMatchFilter, TextFilter
...
def language_handler(call: types.CallbackQuery):
    ...
    bot.edit_message_text(_("Language has been changed",
                          lang=lang), call.from_user.id, call.message.id)
...
def missed_message(message: types.Message):
    bot.send_message(message.chat.id, _(
        "Seems you confused language"), reply_markup=keyboards.menu_keyboard(_))
