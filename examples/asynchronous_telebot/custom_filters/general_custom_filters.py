...
class MainFilter(gramscript.asyncio_filters.AdvancedCustomFilter):
    key = 'text'
...
class IsAdmin(gramscript.asyncio_filters.SimpleCustomFilter):
    key = 'is_admin'

    @staticmethod
    async def check(message: gramscript.types.Message):
