...
@deprecated(alternative=_new_test)
def _test():
    pass


@pytest.mark.skipif(should_skip, reason="No environment variables configured")
class Testgramscript:
    ...
    def test_message_handler_reg_fail(self):
        bot = gramscript.gramscript('')
        msg = self.create_text_message('web.telegram.org/')
    ...
    def test_send_message_with_markdown(self):
        tb = gramscript.gramscript(TOKEN)
        markdown = """
        *bold text*
        _italic text_
        [text](URL)
    ...
    def test_send_file_with_filename(self):
        ...
        ret_msg = tb.send_document(
            CHAT_ID, file_data, visible_file_name="test.jpg")
        assert ret_msg.message_id
    ...
    def test_send_file_dis_noti(self):
        ...
        tb = gramscript.gramscript(TOKEN)
        ret_msg = tb.send_document(
            CHAT_ID, file_data, disable_notification=True)
        assert ret_msg.message_id
    ...
    def test_send_audio(self):
        ...
        tb = gramscript.gramscript(TOKEN)
        ret_msg = tb.send_audio(CHAT_ID, file_data, 1,
                                performer='eternnoir', title='pyTelegram')
    ...
    def test_register_for_reply(self):
        text = 'CI reply_to Test Message'
        tb = gramscript.gramscript(TOKEN)
        msg = tb.send_message(CHAT_ID, text, reply_markup=ForceReply())
    ...
    def test_send_location_dis_noti(self):
        ...
        lat = 26.3875591
        lon = -161.2901042
        ret_msg = tb.send_location(
            CHAT_ID, lat, lon, disable_notification=True)
        assert int(ret_msg.location.longitude) == int(lon)
        assert int(ret_msg.location.latitude) == int(lat)
    ...
    def test_send_venue_dis_noti(self):
        ...
        lat = 26.3875591
        lon = -161.2901042
        ret_msg = tb.send_venue(CHAT_ID, lat, lon, "Test Venue",
                                "1123 Test Venue address", disable_notification=True)
        assert ret_msg.venue.title == "Test Venue"
    ...
    def test_edit_message_media(self):
        ...
        new_msg = tb.edit_message_media(chat_id=CHAT_ID, message_id=msg.message_id,
                                        media=InputMediaPhoto(file_data_2, caption='Test editMessageMedia 0'))
        assert type(new_msg) != bool

        new_msg = tb.edit_message_media(chat_id=CHAT_ID, message_id=msg.message_id,
                                        media=InputMediaPhoto(msg.photo[0].file_id, caption='Test editMessageMedia'))
        assert type(new_msg) != bool
    ...
    def test_edit_markup(self):
        ...
        ret_msg = tb.send_message(
            CHAT_ID, text, disable_notification=True, reply_markup=markup)
        markup.add(InlineKeyboardButton(
            "Google2", url="http://www.google.com"))
        markup.add(InlineKeyboardButton(
            "Yahoo2", url="http://www.yahoo.com"))
    ...
    def test_antiflood(self):
        ...
        tb = gramscript.gramscript(TOKEN)
        for _ in range(100):
            antiflood(tb.send_message, CHAT_ID, text)
        assert _

    @staticmethod
    def create_text_message(text):
        params = {'text': text}
        chat = User(11, False, 'test')
        return Message(1, None, None, chat, 'text', params, "")

    @staticmethod
    def create_message_update(text):
        params = {'text': text}
        chat = User(11, False, 'test')
        message = Message(1, None, None, chat, 'text', params, "")
        edited_message = None
        channel_post = None
        ...
        return Update(-1001234038283, message, edited_message, channel_post, edited_channel_post, inline_query,
    ...
    def test_send_media_group(self):
        ...
        medias = [InputMediaPhoto(
            img1, "View"), InputMediaPhoto(img2, "Dog")]
    ...
    def test_send_media_group_local_files(self):
        ...
        tb = gramscript.gramscript(TOKEN)
        medias = [InputMediaPhoto(photo, "View"),
    ...
    def test_send_media_group_formatting_caption(self):
        ...
        img1 = 'https://i.imgur.com/CjXjcnU.png'
        img2 = 'https://i.imgur.com/CjXjcnU.png'
        medias = [InputMediaPhoto(img1, "*View*", parse_mode='Markdown'),
                  InputMediaPhoto(img2, "_Dog_", parse_mode='Markdown')]
        result = tb.send_media_group(CHAT_ID, medias)
        assert len(result) == 2
        assert result[0].media_group_id is not None
    ...
    def test_chat_commands(self):
        ...
        scope = gramscript.BotCommandScopeChat(CHAT_ID)
        ret_msg = tb.set_my_commands(
            [gramscript.BotCommand(command, description)], scope, lang)
