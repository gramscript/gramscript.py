...
def examine(result, type):
    try:
        ...
        if type == gramscript.namedtuple.Message:
            print('Message glance: %s' %
                  str(gramscript.glance(result, long=True)))
...
def send_everything_on_contact(msg):
    ...
    furl = urlopen('http://i.imgur.com/35HSRQ6.png')
    bot.sendPhoto(chat_id, ('abc.jpg', furl))
    time.sleep(0.5)

    bot.sendPhoto(chat_id, (u'中文照片.jpg', open(
        'lighthouse.jpg', 'rb')), caption=u'中文照片')
    time.sleep(0.5)
