from gramscript.handler.handler_backends import MemoryHandlerBackend, FileHandlerBackend
...
if REDIS_TESTS:
    from gramscript.handler.handler_backends import RedisHandlerBackend


@pytest.fixture()
def telegram_bot():
    return gramscript.gramscript('', threaded=False)


@pytest.fixture
def private_chat():
    return Chat(id=11, type='private')


@pytest.fixture
def user():
    return User(id=10, is_bot=False, first_name='Some User')
...
def reply_to_message_update_type(reply_to_message):
    ...
    chat_member = None
    chat_join_request = None
    return Update(1001234038284, reply_to_message, edited_message, channel_post, edited_channel_post,
...
def test_memory_handler_backend_register_reply_handler(telegram_bot, private_chat, update_type,
    ...
    def start(message):
        message.text = 'entered start'
        telegram_bot.register_for_reply_by_message_id(
            message.message_id, next_handler)

    telegram_bot.process_new_updates([update_type])
    assert update_type.message.text == 'entered start'

    assert len(
        telegram_bot.reply_backend.handlers[update_type.message.message_id]) == 1
...
def test_memory_handler_backend_clear_reply_handler(telegram_bot, private_chat, update_type,
    ...
    telegram_bot.clear_reply_handlers_by_message_id(
        update_type.message.message_id)
