...
from gramscript.api.core import Object, Long, Int
from gramscript.connection import Connection
from gramscript.crypto import IGE, RSA, Prime
...
class Auth:
    ...
    def create(self):
        ...
        while True:
            try:
                log.info(
                    "Start creating a new auth key on DC{}".format(self.dc_id))
                ...
                encrypted_data = RSA.encrypt(
                    data_with_hash, res_pq.server_public_key_fingerprints[0])
                ...
                server_nonce = int.to_bytes(
                    server_nonce, 16, "little", signed=True)
                ...
                log.warning(
                    "Auth key creation failed. Let's try again: {}".format(repr(e)))
                time.sleep(1)
                continue
            else:
                return auth_key
