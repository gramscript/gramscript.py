...
This page contains a list of common words with brief explanations related to gramscript and, to some extent, Telegram in
...
.. glossary::
    ...
    MTProto
        The name of the custom-made, open and encrypted protocol by Telegram, implemented in gramscript.
        :doc:`More on MTProto <topics/mtproto-vs-botapi>`.

    MTProto API
        The Telegram main API gramscript makes use of, which is able to connect both users and normal bots to Telegram
    ...
    gramscriptmer
        A developer that uses gramscript to build Telegram applications.

    Userbot
        Also known as *user bot* or *ubot* for short, is a user logged in by third-party Telegram libraries --- such as
        gramscript --- to automate some behaviours, like sending messages or reacting to text commands or any other event.
    ...
    Decorator
        ...
        Decorators in gramscript are used to automatically register callback functions for handling updates.
