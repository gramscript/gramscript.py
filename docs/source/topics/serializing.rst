...
Serializing means converting a gramscript object, which exists as Python class instance, to a text string that can be
easily shared and stored anywhere. gramscript provides two formats for serializing its objects: one good looking for
...
:doc:`gramscript types <../api/types/index>`, :doc:`raw functions <../telegram/functions/index>` and
...
.. tip::
    ...
    have done that above just to show you how to explicitly convert a gramscript object to JSON.
...
serialize your object, you can use ``eval()`` to get back the original structure; just make sure to ``import gramscript``,
