...
We have just :doc:`installed gramscript <install>`. In this page we'll discuss what you need to do in order to set up a
...
Having the API key from the previous step in handy, we can now begin to configure a gramscript project.
...
-   Alternatively, you can pass your API key to gramscript by simply using the *api_id* and *api_hash* parameters of the
    Client class. This way you can have full control on how to store and load your credentials (e.g., you can load the
    credentials from the environment variables and directly pass the values into gramscript):
