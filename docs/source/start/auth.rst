...
Once a :doc:`project is set up <../intro/setup>`, you will still have to follow a few steps before you can actually use gramscript to make
...
gramscript automatically manages this process, all you need to do is create an instance of the
:class:`~gramscript.Client` class by passing to it a ``session_name`` of your choice (e.g.: "my_account") and call
the :meth:`~gramscript.Client.run` method:
...
After successfully authorizing yourself, a new file called ``my_account.session`` will be created allowing gramscript to
execute API calls with your identity. This file will be loaded again when you restart your app, and as long as you
keep the session alive, gramscript won't ask you again to enter your phone number.
...
:doc:`configure a Telegram API key <../intro/setup>` with gramscript, even when using bots.
