...
This FAQ page provides answers to common questions about gramscript and, to some extent, Telegram in general.
...
**gramscript** is an elegant, easy-to-use Telegram_ client library and framework written from the ground up in Python and
...
The name "gramscript" is composed by **pyro**, which comes from the Greek word *πῦρ (pyr)*, meaning fire, and **gram**,
...
How old is gramscript?
--------------------

gramscript was first released on December 12, 2017. The actual work on the framework began roughly three months prior the
initial public release on `GitHub`_.

.. _GitHub: https://github.com/gramscript/gramscript
...
How stable and reliable is gramscript?
------------------------------------

So far, since its first public release, gramscript has always shown itself to be quite reliable in handling client-server
interconnections and just as stable when keeping long running applications online. The only annoying issues faced are
actually coming from Telegram servers internal errors and down times, from which gramscript is able to recover itself
automatically.

To challenge the framework, the creator is constantly keeping a public
`welcome bot <https://github.com/gramscript/gramscript/blob/develop/examples/welcomebot.py>`_ online 24/7 on his own,
...
:doc:`using gramscript <powered-by>` and is serving more than 200,000 Monthly Active Users since
...
application that implements the MTProto protocol -- just like gramscript -- and uses its own API key, which is always
...
Lots of people ask this question because they are used to the bot API, but things are different in gramscript!

There is no webhook in gramscript, simply because there is no HTTP involved, by default. However, a similar technique is
being used to make receiving updates efficient.

gramscript uses persistent connections via TCP sockets to interact with the server and instead of actively asking for
updates every time (polling), gramscript will simply sit down and wait for the server to send updates by itself
...
Can I use Bot API's file_id values in gramscript?
-----------------------------------------------

Definitely! All file ids you might have taken from the Bot API are 100% compatible and re-usable in gramscript.
...
implements some workarounds server-side to keep backwards compatibility, which gramscript could in turn make use of) and
...
Thanks to `@Manuel15 <https://t.me/gramscriptChat/76990>`_ for the priority list.
...
contact people using official apps. The answer is the same for gramscript too and involves normal usages such as searching
...
that can't be done because the way gramscript deals with handlers would make it hang.

When calling one of the methods above inside an event handler, gramscript needs to wait for all running handlers to finish
in order to safely continue. In other words, since your handler is blocking the execution by waiting for the called
method to finish and since gramscript needs to wait for your handler to finish, you are left with a deadlock.
...
shows up when you try to print something and has very little to do with gramscript itself as it is strictly related to
...
gramscript uses two files that are not Python files, which are not included automatically in the PyInstaller bundle:

- ``gramscript/mime.types``
- ``gramscript/storage/schema.sql``

To fix the issue, you have to locate your local gramscript installation and pass those files to PyInstaller. More info in
...
.. centered:: gramscript is a tool at your commands; it only does what you tell it to do, the rest is up to you.
...
.. raw:: html
    ...
    <script
        ...
        data-telegram-post="gramscriptChat/69424"
        data-width="100%">
    </script>
    <br><br>
...
.. _let me know: https://t.me/gramscript

.. _@smstelegram: https://twitter.com/smstelegram
.. _this form: https://telegram.org/support

.. _Bug Report: https://github.com/gramscript/gramscript/issues/new?labels=bug&template=bug_report.md
